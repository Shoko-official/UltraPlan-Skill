...
- Treat `.ultraplan/` as the private coordination root. All worklogs, plans, issue files, and milestone files live here.
...
```bash
python scripts/bootstrap_ultraplan.py --repo . --profile none
```

The `none` profile creates only the `.ultraplan/` worklog and plan directory, ignoring it via `.git/info/exclude`. This is the correct profile for Antigravity, which loads skills from its own path automatically.
...
2. Gather context.
   ...
   - Read existing contribution docs and project manifests when present.
...
4. Build the private operating system.
   - Create or update `.ultraplan/plan.md`, issue files, milestone files, worklogs, risk register, and decision log as needed.
...
For project-specific automations, create drafts under `.ultraplan/automations/` and keep them ignored.
